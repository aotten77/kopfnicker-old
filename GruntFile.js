...
module.exports = function(grunt){
			...
			cssmin: {
				compress: {
					files: {
						...
						'js/build/describer-config/require-css/owl.carousel.css': ['js/describer-config/require-css/owl.carousel.css'],
	...
	grunt.registerTask('bake-task', function(){
		grunt.initConfig( {
			bake: {
				build: {
					files: [ {
	...
	grunt.registerTask('watchbake-task', function(){
		grunt.initConfig( {
			watch: {
				bake: {
					files: '../demo-sources/**',
					tasks: ['bake-task', 'bake']
				}
			}
		} );
	});
	...
	grunt.registerTask('sass-task', function(){
		grunt.initConfig({
			sass: {
				...
				dist: {
					files: [
						{'.sass-cache/noscript.css': 'scss/noscript.scss'},
						{'.sass-cache/style.css': 'scss/style.scss'}
					]
				}
			}
		});
	...
	grunt.registerTask('autoprefixer-task', function(){
		grunt.initConfig({
			autoprefixer: {
				dist: {
					files: [
						{'css/noscript.css': '.sass-cache/noscript.css'},
						{'css/style.css': '.sass-cache/style.css'}
					]
				}
			}
		});
	...
	grunt.registerTask('watchscss-task', function(){
		grunt.initConfig({
			watch: {
				scss: {
					files: 'scss/**/*.scss',
					tasks: ['sass-task', 'sass']
				},
				css: {
					files: ['.sass-cache/*.css'],
					tasks: ['autoprefixer-task', 'autoprefixer']
				}
			}
		});
	});
	...
	// ohne Compass
	grunt.registerTask('watchscss', ['watchscss-task', 'watch']);
	// SCSS - end

	grunt.registerTask('watchbake', ['watchbake-task', 'watch']);
